"""
Build a rich Excel dashboard from all_shots_full.csv
Sheets:
  1. README          — column glossary
  2. Raw Data        — full dataset with formatted table
  3. Player Summary  — per-player aggregated KPIs
  4. Season Summary  — per-season aggregated KPIs
  5. Archetype Summary
  6. Pressure Analysis
  7. Shot Type Breakdown
  8. Goal Zone Analysis
  9. Top Shots        — top 50 shots by psxG
  10. Dashboard       — key charts embedded
"""
...
# ── Paths ──────────────────────────────────────────────────────────────────
CSV_IN  = "/root/.claude/uploads/d9069c3d-80ef-46d9-baa5-a8e619fea274/6068908a-all_shots_full.csv"
XLSX_OUT = "/home/user/Project-Beth-Mead/wsl_shot_dashboard.xlsx"
...
print(f"  {len(df):,} shots, {df.columns.tolist()}")
...
c = ws["A1"]
c.value     = "WSL Shot Analytics Dashboard — Column Glossary"
...
c = ws["A2"]
c.value     = f"Dataset: {len(df):,} shots  |  Players: {df['player_name'].nunique()}  |  Seasons: {df['season'].nunique()}  |  Goals: {int(df['is_goal'].sum())}"
...
for ri, (col, typ, desc) in enumerate(glossary, 4):
    ...
    is_hdr     = col == "COLUMN"
    row_h      = 18 if not is_section else 16
    ws.row_dimensions[ri].height = row_h
    for ci, val in enumerate([col, typ, desc], 1):
        c = ws.cell(row=ri, column=ci, value=val if not is_section else (col if ci == 1 else ""))
        if is_hdr:
            c.font  = Font(bold=True, size=10, color=C_HEADER_FG, name="Calibri")
            c.fill  = fill(C_SUBHDR_BG)
        elif is_section:
...
for ri, row in enumerate(raw.itertuples(index=False), 2):
    for ci, val in enumerate(row, 1):
        c = ws2.cell(row=ri, column=ci, value=val if not (isinstance(val, float) and np.isnan(val)) else None)
        c.fill      = fill(C_ALT1 if ri % 2 == 0 else C_ALT2)
        c.border    = border()
        c.alignment = left()
        c.font      = body_font(size=9)

# Colour scale for xg column
...
ws2.column_dimensions['A'].width = 12   # season
ws2.column_dimensions['B'].width = 22   # player

# ══════════════════════════════════════════════════════════════════════════════
# SHEET 3: Player Summary
# ══════════════════════════════════════════════════════════════════════════════
print("Sheet: Player Summary...")
...
player_agg['xG_per_Shot']      = player_agg['xG_per_Shot'].round(4)
player_agg['psxG_Total']       = player_agg['psxG_Total'].round(2)
player_agg['psxG_per_Shot']    = player_agg['psxG_per_Shot'].round(4)
player_agg['Goals_minus_xG']   = player_agg['Goals_minus_xG'].round(2)
player_agg['Placement_Qual']   = player_agg['Placement_Qual'].round(4)
...
player_agg['Avg_xGOT']        = player_agg['Avg_xGOT'].round(4)
...
player_agg.columns = [
    ...
    'Under Pressure','Conv%','OT%'
]

n_cols = len(player_agg.columns)
style_header_row(ws3, 1, n_cols)
for ci, col in enumerate(player_agg.columns, 1):
    ws3.cell(row=1, column=ci, value=col)

for ri, row in enumerate(player_agg.itertuples(index=False), 2):
    for ci, val in enumerate(row, 1):
...
for col_name, col_idx in [('Goals−xG', 11), ('Total xG', 7), ('Contact Quality', 13)]:
    ...
    ws3.conditional_formatting.add(
        f"{col_letter}2:{col_letter}{len(player_agg)+1}",
...
season_agg.columns = [
    ...
    'Under Pressure %','Big Chance %','Avg Contact Quality','Conv%','OT%'
]
n_cols = len(season_agg.columns)
style_header_row(ws4, 1, n_cols)
for ci, col in enumerate(season_agg.columns, 1):
    ws4.cell(row=1, column=ci, value=col)
for ri, row in enumerate(season_agg.itertuples(index=False), 2):
    for ci, val in enumerate(row, 1):
...
# ══════════════════════════════════════════════════════════════════════════════
# SHEET 5: Archetype Summary
# ══════════════════════════════════════════════════════════════════════════════
print("Sheet: Archetype Summary...")
...
press_agg['under_pressure'] = press_agg['under_pressure'].map({0:'Not Under Pressure',1:'Under Pressure'})
...
# ══════════════════════════════════════════════════════════════════════════════
# SHEET 7: Shot Type Breakdown
# ══════════════════════════════════════════════════════════════════════════════
print("Sheet: Shot Type Breakdown...")
ws7 = wb.create_sheet("⚽ Shot Types")
ws7.sheet_view.showGridLines = False

type_dims = ['body_part','outcome','archetype']
row_offset = 1
...
style_header_row(ws9, 1, n_cols)

# Title row
ws9.insert_rows(1)
ws9.merge_cells(f"A1:{get_column_letter(n_cols)}1")
c = ws9["A1"]
c.value     = "Top 100 Shots by Post-Shot xG"
...
# ══════════════════════════════════════════════════════════════════════════════
# SHEET 10: Dashboard (KPI cards + charts)
# ══════════════════════════════════════════════════════════════════════════════
print("Sheet: Dashboard...")
ws_dash = wb.create_sheet("📈 Dashboard", 0)  # first sheet
...
c = ws_dash["A1"]
c.value     = "WSL Shot Analytics Dashboard"
...
c = ws_dash["A2"]
c.value     = f"Dataset: {len(df):,} shots  |  {df['player_name'].nunique()} players  |  {df['season'].nunique()} seasons  |  {int(df['is_goal'].sum())} goals"
...
kpis = [
    ("Total Shots",    f"{len(df):,}",                    None),
    ("Total Goals",    f"{int(df['is_goal'].sum()):,}",   None),
    ("Total xG",       f"{df['xg'].sum():.1f}",           None),
    ("Goals − xG",     f"{(df['is_goal'].sum()-df['xg'].sum()):.1f}", None),
    ("Avg xG/Shot",    f"{df['xg'].mean():.4f}",          None),
    ("Avg psxG/Shot",  f"{df['psxg'].mean():.4f}",        None),
    ("On-Target %",    f"{df['is_on_target'].mean()*100:.1f}%", None),
    ("Conversion %",   f"{df['is_goal'].mean()*100:.1f}%",None),
    ("Big Chances",    f"{int(df['is_big_chance'].sum()):,}",None),
    ("Avg Distance",   f"{df['distance'].mean():.1f} m",  None),
]
...
ws_dash.row_dimensions[kpi_row+3].height = 12  # gap row

# ── Chart 1: Goals by Outcome ──
outcome_data = df['outcome'].value_counts().reset_index()
outcome_data.columns = ['Outcome','Count']
chart_start_row = kpi_row + 5

# Write chart data to a hidden area (far right)
data_col = 22
...
pie.style = 10
labels  = Reference(ws_dash, min_col=data_col,   min_row=chart_start_row+1, max_row=chart_start_row+len(outcome_data))
data_ref= Reference(ws_dash, min_col=data_col+1, min_row=chart_start_row,   max_row=chart_start_row+len(outcome_data))
pie.add_data(data_ref, titles_from_data=True)
pie.set_categories(labels)
pie.width  = 12; pie.height = 10
ws_dash.add_chart(pie, "A9")

# ── Chart 2: xG by Archetype ──
arch_chart_data = df.groupby('archetype')[['xg','is_goal']].agg({'xg':'sum','is_goal':'sum'}).reset_index()
arch_chart_data.columns = ['Archetype','Total xG','Goals']
arch_chart_data = arch_chart_data.sort_values('Total xG', ascending=False)
...
# ── Chart 3: xG/Shot by Season ──
season_xg = df.groupby('season')[['xg','is_goal']].mean().reset_index()
season_xg.columns = ['Season','Avg xG/Shot','Avg Goal Rate']
season_xg = season_xg.sort_values('Season')
...
line = LineChart()
line.title   = "Avg xG/Shot & Goal Rate by Season"
...
# ── Chart 4: Top 15 Players by xG ──
top15 = df.groupby('player_name')['xg'].sum().nlargest(15).reset_index()
top15.columns = ['Player','Total xG']
top15 = top15.sort_values('Total xG')
...
bar2 = BarChart()
bar2.type    = "bar"
bar2.title   = "Top 15 Players — Total xG"
...
# ── Chart 6: Goals vs xG scatter (bar proxy) — Top 20 by shots ──
top20 = df.groupby('player_name').agg(
    xG = ('xg','sum'), Goals = ('is_goal','sum')
).reset_index().nlargest(20, 'xG').sort_values('xG')
...
bar4 = BarChart()
bar4.type    = "bar"
bar4.title   = "Goals vs xG — Top 20 Players"
...
# ══════════════════════════════════════════════════════════════════════════════
# SHEET 11: Player vs League Percentiles
# ══════════════════════════════════════════════════════════════════════════════
...
metric_labels = ['xG/Shot','psxG/Shot','Situation Danger','P(On Target)','xGOT',
                 'Placement Quality','Contact Quality','Shot Power',
...
style_header_row(ws_pct, 1, n_cols)
for ci, col in enumerate(combined.columns, 1):
    ws_pct.cell(row=1, column=ci, value=col)
for ri, row in enumerate(combined.itertuples(index=False), 2):
    for ci, val in enumerate(row, 1):
        c = ws_pct.cell(row=ri, column=ci, value=val)
        c.fill = fill(C_ALT1 if ri % 2 == 0 else C_ALT2)
        c.border = border(); c.alignment = left(); c.font = body_font(size=9)

# Colour scale every percentile column (odd columns after Player)
for col_i, col_name in enumerate(combined.columns[1:], 2):
    if '(raw)' not in col_name:
        col_letter = get_column_letter(col_i)
        ws_pct.conditional_formatting.add(
            f"{col_letter}2:{col_letter}{len(combined)+1}",
            ColorScaleRule(start_type='min', start_color=C_BAD_LIGHT,
                           mid_type='percentile', mid_value=50, mid_color='FFFFFF',
                           end_type='max', end_color=C_GOOD_LIGHT)
        )
